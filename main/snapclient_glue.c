...
{
    if (!s_network_available || s_paused || !s_run) {
        errno = ENETDOWN;
        return -1;
    }
    ...
    int port_length = snprintf(port_text,
                               sizeof(port_text),
                               "%u",
                               (unsigned)s_port);
    ...
    int socket_fd = socket(result->ai_family,
                           result->ai_socktype,
                           result->ai_protocol);
    if (socket_fd < 0) {
    ...
    if (original_flags < 0 ||
        ...
        ESP_LOGW(TAG, "Socket konnte nicht auf nonblocking gesetzt werden, errno=%d", errno);
        close(socket_fd);
        freeaddrinfo(result);
        return -1;
    }
    ...
    if (connect_result != 0 && errno != EINPROGRESS) {
        ESP_LOGW(TAG,
                 "connect() zu %s:%u sofort fehlgeschlagen (errno=%d)",
                 s_host,
                 (unsigned)s_port,
                 errno);
        close(socket_fd);
    ...
    if (connect_result != 0) {
        ...
        int select_result = select(socket_fd + 1,
                                   NULL,
                                   &write_set,
                                   &error_set,
                                   &timeout);
        if (select_result <= 0) {
            int saved_errno = select_result == 0 ? ETIMEDOUT : errno;
            ESP_LOGW(TAG,
                     "TCP-Verbindungsaufbau zu %s:%u abgebrochen (%s, errno=%d)",
                     s_host,
                     (unsigned)s_port,
                     select_result == 0 ? "Timeout" : "select-Fehler",
                     saved_errno);
            close(socket_fd);
            freeaddrinfo(result);
            errno = saved_errno;
        ...
        if (getsockopt(socket_fd,
            ...
            ESP_LOGW(TAG,
                     "TCP-Verbindungsaufbau zu %s:%u fehlgeschlagen (errno=%d)",
                     s_host,
                     (unsigned)s_port,
                     socket_error);
            close(socket_fd);
    ...
    if (fcntl(socket_fd, F_SETFL, original_flags) < 0) {
        ESP_LOGW(TAG, "Socket konnte nicht auf blocking zurueckgesetzt werden, errno=%d", errno);
        close(socket_fd);
        return -1;
    }
...
static void wait_for_reconnect_condition(TickType_t timeout_ticks)
{
    ...
    xEventGroupWaitBits(s_evt,
                        EVT_RESUME | EVT_NETWORK_AVAILABLE | EVT_STATE_CHANGED,
                        pdTRUE,
                        pdFALSE,
                        timeout_ticks);
}
...
static void snap_task(void *argument)
{
    ...
    while (s_run) {
        if (s_paused || !s_network_available) {
            EventBits_t required = EVT_NETWORK_AVAILABLE;
            if (s_paused) {
                required |= EVT_RESUME;
            }
        ...
        if (socket_fd < 0) {
            if (s_run && !s_paused && s_network_available) {
                wait_for_reconnect_condition(
        ...
        if (!s_paused && s_run && s_network_available) {
            ESP_LOGI(TAG,
...
void snapclient_set_network_available(bool available)
{
    ...
    if (available) {
        ESP_LOGI(TAG, "NET UP: Snapclient-Reconnect sofort freigegeben");
        if (s_evt != NULL) {
            xEventGroupSetBits(s_evt,
                               EVT_NETWORK_AVAILABLE | EVT_STATE_CHANGED);
        }
        return;
    }

    ESP_LOGW(TAG, "NET DOWN: laufende Snapserver-Verbindung wird abgebrochen");
    if (s_evt != NULL) {
        xEventGroupClearBits(s_evt, EVT_NETWORK_AVAILABLE);
        xEventGroupSetBits(s_evt, EVT_STATE_CHANGED);
    }

    int socket_fd = s_sock;
    if (socket_fd >= 0) {
        shutdown(socket_fd, SHUT_RDWR);
    }
}
